...
module.exports = {
  listeners: {
      ...
      if (this.party) {
        ...
        for (const member of this.party) {
          ...
          if (damage.source !== damage.attacker) {
            ...
            const damageSourceName = damage.source.name[0].toUpperCase() + damage.source.name.slice(1);
    ...
    healed: (state) => function (heal, finalAmount) {
      ...
      } else if (heal.source.gender === 'male') {
      ...
      for (const member of this.party) {
        ...
        const buf = `${source}${attacker} вылечило ${this.dname} <b><red>${finalAmount}</red></b>.`;
        B.sayAt(member, buf);
      }
    },
    ...
    killed: (state) => {
      ...
      return function (killer) {
        ...
        const helthDamaged = Math.floor(this.getMaxAttribute('health') * 0.8);
        const manaDamaged = Math.floor(this.getMaxAttribute('mana') * 0.8);
        ...
        this.moveTo(home, (_) => {
          ...
          //this.save();
          //приходится выводить персонажа из игры, т.к. после смерти возникает баг,
          //игрок не может нормально сражаться
...
function promptBuilder(promptee) {
  ...
  for (const target of promptee.combatants) {
    currentPerc = Math.floor((target.getAttribute('health') / target.getMaxAttribute('health')) * 100);
    progress = B.progress(progWidth, currentPerc, 'red');
